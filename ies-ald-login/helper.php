...
{
    //+ Modify by eporquenon
    return trim(exec("grep " . escapeshellarg($clientIP) . " /tmp/dhcp.leases | awk '{print $2}'"));
////++    return trim(exec("grep " . escapeshellarg($clientIP) . " /var/log/syslog | grep dnsmasq-dhcp | grep DHCPREQUEST | awk '{print \$NF}' | sort -u"));
}
...
////+    return trim(exec("grep " . escapeshellarg($clientIP) . " /var/log/syslog | grep dnsmasq-dhcp | grep DHCPACK | awk '{print \$NF}' | sort -u"));
////+    return $clientIP;
}
